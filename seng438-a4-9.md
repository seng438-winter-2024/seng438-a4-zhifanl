...
|-------------|------|-----------------------------------------------------------------------------------|-------------------|
| 1           | 176  | Replaced boolean return with false for `org/jfree/data/Range::intersects`         | Our test suite successfully detected and killed this mutation, indicating the test is effective at validating true conditions for intersects method. |
| 2           | 176  | Replaced boolean return with true for `org/jfree/data/Range::intersects`          | This mutation was killed by the test suite, showing the our tests are capable of identifying false positives in intersects validation. |
| 3           | 176  | Removed call to `org/jfree/data/Range::getLowerBound`                             | The mutation was killed, suggesting that our tests depend on the lower bound for determining range intersections accurately. |
| 4           | 176  | Removed call to `org/jfree/data/Range::getUpperBound`                             | Killing this mutation highlights the importance of the upper bound in the test's logic for checking range intersections. |
| 5           | 176  | Removed call to `org/jfree/data/Range::intersects`                                | The tests killed this mutation, proving that the direct intersection method is crucial for determining if ranges intersect. |
| 6           | 176  | Replaced return of integer sized value with `(x == 0 ? 1 : 0)`                    | This mutation was killed, indicating the tests are effectively validating the method's logic and handling of integer-sized values. |
