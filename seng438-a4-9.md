...
This lab report focuses on the exploration and understanding of two key testing methodologies: Mutation Testing and GUI Testing, applied to a selected website. The first segment of the assignment required us to delve into the intricacies of Mutation Testing. This involved the use of a tool known as PIT Mutations to introduce mutation faults into a Java code-base. The task was to interpret the mutation scores reported and analyze the mutants that were either killed or survived. This analysis was then used to devise new test cases aimed at enhancing the overall efficacy of the test suite. The second part of the assignment revolved around GUI Testing of a chosen website. This was accomplished through a prevalent approach to GUI test automation known as record and replay. The exercise provided an opportunity to learn and apply Selenium, a widely recognized tool for web interface testing, and compare its performance with other alternatives.

# Analysis of 10 Mutants of the Range class 

1. (line176) replaced boolean return with false for org/jfree/data/Range::intersects → KILLED

This mutation changes the method to always return false, suggesting the ranges never intersect. The mutation was killed, meaning our tests correctly identified that this change introduced a fault.

2. (line176) replaced boolean return with true for org/jfree/data/Range::intersects → KILLED

Opposite to the first, this mutation always returns true, indicating the ranges always intersect. It was also killed, showing our tests can detect this unrealistic scenario.

3. (line176) removed call to org/jfree/data/Range::getLowerBound → KILLED

This mutation omits the action of fetching the lower bound of a range, potentially disrupting the intersection logic. Our tests determined intersection correctly and killed this mutant

4. (line176) removed call to org/jfree/data/Range::getUpperBound → KILLED

Similar to the previous one, but for the upper bound. Its detection as killed indicates the upper bound is also crucial for the intersection logic in our tests.

5. (line176) removed call to org/jfree/data/Range::intersects → KILLED

This mutation eliminates the intersection logic entirely. The fact that it was killed confirms our test suite is designed to verify the intersection behavior explicitly.

6. (line176) replaced return of integer sized value with (x == 0 ? 1 : 0) → KILLED

This generic description implies an integer-returning method was altered to return 1 if its original return was 0, and 0 otherwise. 
...
Range.java: After we tested the file file from Assignmet 3, we got a 58 mutation score, which was not ideal. Then we added a bunch of test cases to kill the alive mutants. AFter that we increased the mutation score by 12%. The updated rangetest.java has a 70% mutation score.

DataUtilitieskava: After we tested the file file from Assignmet 3, we got a 81 mutation score, which was already perfect. Then we added a bunch of test cases to kill the remaining alive mutants. AFter that we increased the mutation score by 11%. The updated rangetest.java has a 92% mutation score. 
