...
|-------------|------|----------------------|-------------------|--------|
| 1 | 176 | Replaced boolean return with false | This mutation alters the `intersects` method to consistently return `false`, artificially indicating that no ranges intersect, regardless of their actual positions or lengths. By marking this mutation as "KILLED", our testing framework has proven its ability to detect even the most subtle errors—those that negate the entire purpose of a method designed to determine the intersection between ranges. This ensures that our software reliably identifies overlapping ranges, a critical feature for functionalities dependent on range comparisons. | KILLED |
