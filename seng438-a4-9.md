...
range.java: After we test the file file from Assignmet 3, we got a 58 mutation score, which is not ideal. Then we added a bunch of test cases to kill the alive mutants. AFter that we increased the mutation score by 12%. The updated rangetest.java has a 70% mutation score.

datautilitieskava: After we test the file file from Assignmet 3, we got a 81 mutation score, which is already perfect. Then we added a bunch of test cases to kill the remaining alive mutants. AFter that we increased the mutation score by 11%. The updated rangetest.java has a 92% mutation score. 
