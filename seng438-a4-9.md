...
| Mutation ID | Line | Mutation Description                                                                                     | Detailed Analysis                                                                                      |
|-------------|------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|
| 1           | 176  | Replaced boolean return with false for org/jfree/data/Range::intersects → KILLED                         | This mutation changes the return value to always false. The test suite killed it, indicating that it can effectively identify incorrect return values when determining if ranges intersect. |
| 2           | 176  | Replaced boolean return with true for org/jfree/data/Range::intersects → KILLED                          | This mutation changes the return value to always true. The test suite killed it, showing it is capable of detecting when the method incorrectly identifies all ranges as intersecting. |
| 3           | 176  | Removed call to org/jfree/data/Range::getLowerBound → KILLED                                              | Removing the call to get the lower bound disrupts the method's ability to accurately determine intersections. The test suite killing this mutation demonstrates its reliance on accurate lower bound values. |
| 4           | 176  | Removed call to org/jfree/data/Range::getUpperBound → KILLED                                              | This mutation eliminates the method's access to the upper bound, which is critical for accurate intersection checks. The test suite's ability to kill this mutation shows it tests the upper bound's role effectively. |
| 5           | 176  | Removed call to org/jfree/data/Range::intersects → KILLED                                                 | By removing the intersects method call, this mutation significantly alters the logic for checking range intersections. The test suite killing this mutation proves its effectiveness in validating the correctness of the intersects logic. |
| 6           | 176  | Replaced return of integer sized value with (x == 0 ? 1 : 0) → KILLED                                     | This mutation alters how integer values are handled, potentially impacting the method's logic. The test suite killing it indicates that the tests are sensitive to the precise handling of integer values within the method. |
